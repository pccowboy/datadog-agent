...
func TestNetworkConnectionBatching(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		ttm := &model.NetworkTracerTelemetry{}
		chunks := batchConnections(cfg, 0, tc.cur, map[string]*model.DNSEntry{}, "nid", ctm, nil, ttm)
		...
		for i, c := range chunks {
			...
			if i == 0 {
				...
				assert.NotNil(t, connections.TracerTelemetry)
			} else {
				assert.Nil(t, connections.ConnTelemetry)
				assert.Nil(t, connections.TracerTelemetry)
			}
		}
...
func TestNetworkConnectionBatchingWithDomains(t *testing.T) {
	...
	chunks := batchConnections(cfg, 0, conns, dns, "nid", nil, domains, nil)
