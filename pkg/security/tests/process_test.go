...
func TestProcessContext(t *testing.T) {
	...
	t.Run("args-overflow", func(t *testing.T) {
		...
		if err != nil {
			...
			assert.Equal(t, len(args.([]string)), 2, "incorrect number of args")
			assert.Equal(t, strings.HasSuffix(args.([]string)[1], "..."), true, "args not truncated")
		}
	...
	t.Run("tty", func(t *testing.T) {
		...
		if err != nil {
			...
			assertFieldEqual(t, event, "process.file.path", executable)
			assert.Equal(t, event.ProcessContext.FileFields.Inode, getInode(t, executable), "wrong inode")
