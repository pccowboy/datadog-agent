...
//export HelloWorld
func HelloWorld(value *C.char) {
    fmt.Printf(C.GoString(value))
}
...
func SendTraces(value unsafe.Pointer, length C.int){
	...
	fmt.Println("Writing to agent In channel")
	if (agnt == nil){
		fmt.Println("Agnt is nil")
	}
...
func Init() {
	...
	// Invoke the Agent
	cfg, err := config.Load("C:\\ProgramData\\Datadog\\datadog.yaml")

	if (err == nil) {
		fmt.Println("No error in configuration")
	} else {
		fmt.Printf("%s", err)
	}
